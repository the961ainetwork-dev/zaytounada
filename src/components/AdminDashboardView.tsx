...
export default function AdminDashboardView({ onRestaurantsUpdated }: AdminDashboardViewProps) {
  ...
  const [adminTab, setAdminTab] = useState<'reservations' | 'restaurants' | 'subscribers' | 'emails' | 'stories' | 'pages-sections' | 'suppliers' | 'menus'>('reservations');
  ...
  const fetchAllData = async () => {
    setIsLoading(true);
    try {
      const [resB, resR, resS, resE, resA, resP, resSettings, resSuppliers, resMenus] = await Promise.all([
      ...
      if (Array.isArray(resMenus)) setMenus(resMenus);
  ...
  // --- MENU ACTIONS ---
  const handleUpdateMenuStatus = async (id: string, status: 'approved') => {
    try {
      ...
      if (res.ok) {
        triggerAlert('success', `Menu vetted and successfully published to the live listings!`);
        fetchAllData();
      } else {
        triggerAlert('err', 'Failed to approve menu.');
      }
    } catch (err) {
      triggerAlert('err', 'Network error approving menu.');
    }
  };

  const handleDeleteMenu = async (id: string) => {
    if (!confirm('Are you absolutely sure you want to delete this menu listing? This action is irreversible.')) return;
    try {
      const res = await fetch(`/api/menus/${id}`, { method: 'DELETE' });
      if (res.ok) {
        triggerAlert('success', 'Menu listing successfully deleted.');
        fetchAllData();
      } else {
        triggerAlert('err', 'Failed to delete menu listing.');
      }
    } catch (err) {
      triggerAlert('err', 'Error connecting to menus API.');
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap gap-2 border-b border-neutral-200 pb-3 mb-6">
          ...
          <button
            ...
            id="admin-menus-tab-btn"
          >
            <FileText className="w-4 h-4 text-amber-500" />
            <span>Menus Vetting ({menus.filter(m => m.status === 'pending').length})</span>
          </button>
        </div>
        ...
        {/* TABLE CONTROL: MENUS VETTING AND MANAGEMENT */}
        {adminTab === 'menus' && (
          <div className="bg-white border border-neutral-200 rounded-2xl shadow-sm overflow-hidden animate-fade-in text-left">
            <div className="p-4 border-b border-neutral-100 bg-neutral-50/50 flex flex-col sm:flex-row justify-between items-center gap-4">
              <div>
                <span className="text-xs font-mono font-bold text-neutral-800 uppercase block tracking-wider">
                  📖 Resto & Pub Menus Vetting & Approval Queue ({menus.length})
                </span>
                <span className="text-[10px] text-neutral-400 block mt-0.5">
                  Review and authorize menu questionnaires submitted by restaurant/pub owners, or delete existing directories.
                </span>
              </div>
              <div className="text-[10px] font-mono bg-emerald-800 text-amber-300 font-bold px-3 py-1 rounded">
                SECURE MENU MANAGEMENT
              </div>
            </div>
            ...
            {menus.length === 0 ? (
              <div className="p-16 text-center text-neutral-500">
                ...
                <h4 className="font-semibold text-neutral-900">No menus registered.</h4>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-neutral-50 border-b border-neutral-150 text-[10px] font-mono uppercase tracking-wider text-neutral-600">
                      ...
                      <th className="px-6 py-4">Cuisine & Price</th>
                      <th className="px-6 py-4">Submitted By (Owner)</th>
                      <th className="px-6 py-4">Menu Content Sections</th>
                  ...
                  <tbody className="divide-y divide-neutral-100 text-xs">
                    {menus.map((menu) => (
                      <tr key={menu.id} className="hover:bg-neutral-50/60 transition-colors">
                        <td className="px-6 py-4">
                          <div className="font-serif font-black text-neutral-900 text-sm uppercase">{menu.restaurantName}</div>
                          <div className="flex items-center gap-1.5 mt-1 text-[10px] text-neutral-500 font-mono font-bold uppercase">
                            <MapPin className="w-3.5 h-3.5" />
                            <span>{menu.city}</span>
                          </div>
                        ...
                        <td className="px-6 py-4">
                          <span className="text-amber-700 bg-amber-50 border border-amber-150 rounded px-2 py-0.5 text-[10px] font-mono font-bold block w-fit mb-1">
                            {menu.cuisine}
                          </span>
                          <span className="font-mono text-emerald-800 font-bold bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 text-[10px]">
                            {menu.priceRange}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          {menu.submittedBy ? (
                            <div>
                              <div className="font-semibold text-neutral-900">{menu.submittedBy.ownerName}</div>
                              <div className="text-neutral-500 font-mono text-[10px]">{menu.submittedBy.email}</div>
                              <div className="text-neutral-500 font-mono text-[10px]">{menu.submittedBy.phone}</div>
                            </div>
                          ) : (
                            <span className="text-neutral-400 italic text-[10px]">Zaytounada Curator</span>
                          )}
                        </td>
                        <td className="px-6 py-4">
                          <div className="space-y-2 max-w-sm">
                            {menu.sections.map((section: any, sIdx: number) => (
                              <div key={sIdx} className="bg-neutral-50 p-2 rounded border border-neutral-150" id={`admin-menu-sec-${menu.id}-${sIdx}`}>
                                <span className="font-mono text-[9px] font-bold text-neutral-500 block uppercase mb-1">
                                  {section.category} ({section.items.length} items)
                                </span>
                                <div className="text-[10.5px] text-neutral-700 font-medium truncate">
                                  {section.items.map((it: any) => it.name).join(', ')}
                                </div>
                              </div>
                            ))}
                          </div>
                        </td>
                        <td className="px-6 py-4 font-mono text-neutral-450">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono tracking-wider font-extrabold uppercase ${
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            {menu.status !== 'approved' && (
                              <button
                                onClick={() => handleUpdateMenuStatus(menu.id, 'approved')}
                                className="p-1 px-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-mono text-[9px] uppercase font-bold rounded shadow-xs transition-colors cursor-pointer"
                              >
                                APPROVE
                              </button>
                            )}
