...
export default function MapView({
  ...
  const bgStyle = useMemo(() => {
    if (mapMode === 'satellite') {
      return 'bg-[#020504] bg-[radial-gradient(ellipse_at_70%_50%,_#09241c_0%,_#030f0b_45%,_#010302_100%)]';
    }
    return cityMapThemes.bg; // bg-[#030303]
    ...
    switch (currentCityForMap) {
      case 'Beirut':
        return {
          ...
          satelliteZones: [
            { name: 'Horsh Beirut Forest', x: 44, y: 74, w: 22, h: 14, color: 'bg-emerald-950/40 border-emerald-800/20' },
            { name: 'Downtown Pedestrian Quarter', x: 52, y: 48, w: 10, h: 7, color: 'bg-emerald-950/35 border-emerald-800/20' }
          ]
        };
      case 'Byblos':
        return {
          ...
          satelliteZones: [
            { name: 'Archaeological Excavation Area', x: 55, y: 45, w: 15, h: 15, color: 'bg-emerald-950/30 border-emerald-850/20' }
          ]
        };
      case 'Batroun':
        return {
          ...
          satelliteZones: [
            { name: 'Ancient Phoenician Sea Wall Reserve', x: 28, y: 52, w: 10, h: 22, color: 'bg-sky-950/30 border-sky-800/20' }
          ]
        };
      case 'Tripoli':
        return {
          ...
          satelliteZones: [
            { name: 'Historic Grand Souk Alleys', x: 45, y: 48, w: 20, h: 15, color: 'bg-amber-950/30 border-amber-800/20' }
          ]
        };
      default:
        return {
          ...
          satelliteZones: [
            { name: 'Municipal Park', x: 45, y: 45, w: 10, h: 10, color: 'bg-emerald-950/40 border-emerald-800/20' }
          ]
        };
    }
  }, [currentCityForMap]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" id="gastronomic-map-view">
      {/* Title block */}
            ...
            <button
              ...
              className={`px-4 py-2 text-xs font-semibold rounded transition-all cursor-pointer ${
                currentCityForMap === city
                  ? 'bg-transparent border border-red-600 text-red-500 font-bold'
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 relative bg-[#0a0a0a] border border-white/5 rounded-xl p-4 overflow-hidden shadow-2xl flex flex-col min-h-[500px]">
          {/* Map Controls */}
          <div className="absolute top-6 left-6 z-20 flex flex-col gap-2 bg-black/85 border border-white/5 p-3 rounded shadow-2xl backdrop-blur-md text-left">
            <span className="text-[9px] font-mono tracking-widest text-red-500 uppercase font-bold">Chart Status</span>
            <h3 className="font-serif font-light text-sm text-white leading-tight uppercase tracking-wider">
              {cityMapThemes.title}
            </h3>
            <p className="text-[10px] text-white/40 font-light">
              {cityMapThemes.subtitle}
            </p>
            <div className="flex items-center gap-4 mt-2 border-t border-white/5 pt-2 text-[9px] font-mono uppercase tracking-wider text-white/60 font-medium">
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input 
                  type="checkbox" 
                  checked={showGrid} 
                  onChange={() => setShowGrid(!showGrid)} 
                  className="rounded border-[#555] bg-transparent text-red-600 focus:ring-0"
                />
                <span>Gridlines</span>
              </label>
              <button 
                ...
                className="flex items-center gap-1 hover:text-red-500 select-none cursor-pointer text-white/40 hover:text-white transition-colors ml-auto"
              >
            ...
            <div className="border-t border-white/5 pt-2.5 space-y-2.5 mt-1">
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-mono tracking-widest text-[#d97706] uppercase font-bold flex items-center gap-1">
                  <Ruler className="w-2.5 h-2.5" />
                  <span>Distance Ruler</span>
                </span>
                {isMeasuring && (
                  <span className="text-[7.5px] font-mono text-emerald-400 bg-emerald-950/40 px-1 border border-emerald-800/35 animate-pulse uppercase rounded">Active</span>
                )}
              </div>
              <button
                onClick={() => {
                ...
                className={`w-full flex items-center justify-center gap-1.5 px-2.5 py-1.5 rounded text-[9.5px] uppercase tracking-wider font-semibold border transition-all cursor-pointer ${
                  isMeasuring 
                    ? 'border-amber-500/40 text-amber-400 bg-amber-950/20 shadow-[0_0_10px_rgba(245,158,11,0.1)]' 
                    : 'border-white/5 text-white/80 hover:bg-white/5 bg-[#0e0e0e]'
                }`}
              ...
              {isMeasuring && (
                <div className="text-[9.5px] leading-relaxed pt-0.5 text-white/50 font-light space-y-1.5 min-w-[185px]">
                  {!measureStart ? (
                    <p className="text-white/40 italic text-[9px]">Click point A on the map to start...</p>
                  ) : !measureEnd ? (
                    <p className="text-white/40 italic text-[9px] animate-pulse">Click point B to calculate...</p>
                  ) : (
                    <div className="bg-[#0c0c0c]/90 border border-amber-500/20 p-2.5 rounded space-y-1.5 font-mono text-[9px] tracking-wide text-white/90">
                      <div className="flex justify-between items-center text-white border-b border-white/5 pb-1">
                        <span className="text-amber-400 font-bold uppercase text-[8px]">GEODESIC METRICS</span>
                        <button 
                      ...
                      <div className="mt-1">
                        <p className="font-serif text-[13px] font-semibold text-white tracking-normal flex items-baseline gap-1">
                          {distanceDisplay.km} km <span className="text-[9px] font-mono text-white/45">({distanceDisplay.meters} m)</span>
                        </p>
                      </div>
                      <div className="text-white/55 text-[8.5px] flex flex-col gap-0.5 mt-1 border-t border-white/5 pt-1.5 uppercase font-medium">
                        <p>🚶 Walking: ~{distanceDisplay.walkTime} mins</p>
                        <p>🚗 Driving: ~{distanceDisplay.driveTime} mins</p>
                      </div>
                      <div className="text-[8px] text-amber-400/70 border-t border-white/5 pt-1.5 flex flex-col gap-0.5 mt-1 font-light">
                        <div className="truncate"><span className="font-bold text-amber-500">A</span>: {measureStart.name || `Point (${Math.round(measureStart.x)}%, ${Math.round(measureStart.y)}%)`}</div>
                        <div className="truncate"><span className="font-bold text-red-500">B</span>: {measureEnd.name || `Point (${Math.round(measureEnd.x)}%, ${Math.round(measureEnd.y)}%)`}</div>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="absolute bottom-6 left-6 z-20 flex flex-col gap-2.5 bg-black/90 border border-white/5 p-3.5 rounded shadow-2xl backdrop-blur-md text-left min-w-[190px] max-w-[210px]">
            <span className="text-[9px] font-mono tracking-widest text-red-500 uppercase font-extrabold select-none">Zaytouynda Legend</span>
            <div className="space-y-2">
              <div className="flex items-center gap-2.5">
                <div className="w-6 h-6 rounded-full bg-red-600 border border-white/20 flex items-center justify-center shrink-0 shadow-sm">
                  <span className="text-amber-300 font-bold text-[10px] leading-none mb-0.5 select-none">✻</span>
                </div>
                <div>
                  <h4 className="text-[9.5px] uppercase font-bold font-mono text-white/90">Zaytouynda Star</h4>
                  <p className="text-[8.5px] text-white/40 font-light mt-0.5 leading-tight">Elite cuisine of excellent status.</p>
                </div>
              </div>
              <div className="flex items-center gap-2.5">
                <div className="w-6 h-6 rounded-full bg-amber-600 border border-white/15 flex items-center justify-center shrink-0 shadow-sm">
                  <span className="text-white text-xs font-bold font-sans select-none">☺</span>
                </div>
                <div>
                  <h4 className="text-[9.5px] uppercase font-bold font-mono text-white/90">Bib Gourmand</h4>
                  <p className="text-[8.5px] text-white/40 font-light mt-0.5 leading-tight">High value cooking, moderate prices.</p>
                </div>
              </div>
              <div className="flex items-center gap-2.5">
                <div className="w-6 h-6 rounded-full bg-stone-800 border border-white/10 flex items-center justify-center shrink-0 shadow-sm">
                  <span className="text-white text-[9px] font-bold font-serif select-none">Z</span>
                </div>
                <div>
                  <h4 className="text-[9.5px] uppercase font-bold font-mono text-white/90">Selected Table</h4>
                  <p className="text-[8.5px] text-white/40 font-light mt-0.5 leading-tight">Inspector-vetted quality kitchen.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="absolute bottom-6 right-6 z-10 pointer-events-none opacity-20 sm:opacity-30">
            <div className="relative w-24 h-24 border border-white/10 rounded-full flex items-center justify-center animate-spin-slow text-white/20">
          ...
          <div 
            ...
            className={`relative flex-1 rounded-lg overflow-hidden border border-white/5 ${bgStyle} transition-all duration-300 min-h-[440px] ${isMeasuring ? 'cursor-crosshair' : ''}`}
          >
            
            {/* Map Mode Toggles inside Canvas */}
            <div className="absolute top-4 right-4 z-30 flex bg-black/90 border border-white/10 p-1 rounded backdrop-blur-md shadow-lg" id="map-mode-selector">
              <button
                onClick={(e) => { e.stopPropagation(); setMapMode('street'); }}
                className={`flex items-center gap-1.5 px-3 py-1.5 text-[9px] font-mono uppercase tracking-widest font-bold transition-all cursor-pointer rounded ${
                  mapMode === 'street'
            ...
            {/* Map Imagery Mesh Layer (with dynamic CSS filter effects to simulate terrain depth and elevation sensor rendering) */}
            <div 
              className="absolute inset-0 pointer-events-none transition-all duration-500"
              style={{
                filter: mapMode === 'satellite' 
                  ? 'contrast(1.22) saturate(1.15) brightness(0.88) hue-rotate(-2deg)' 
                  : 'none'
              }}
            >
              ...
              {cityMapThemes.landmarks.find(l => l.type === 'river') && (
                <div className={`absolute inset-x-0 top-1/2 -translate-y-1/2 h-14 blur-[2px] border-y transform -rotate-12 pointer-events-none flex items-center justify-center transition-all ${
                  mapMode === 'satellite'
                    ? 'bg-sky-950/25 border-sky-800/10'
                    : 'bg-red-500/5 border-red-500/10'
                }`}>
                  <span className={`font-serif italic text-xs tracking-widest uppercase font-bold transition-colors ${
                    mapMode === 'satellite'
                      ? 'text-sky-450/30'
                      : 'text-red-500/15'
                  }`}>
                  ...
                  <div className="absolute inset-0 pointer-events-none">
                    {cityDetailedFeatures.streets.map((street, idx) => (
                      <div
                        ...
                        className="absolute -translate-x-1/2 -translate-y-1/2 text-[7px] font-mono tracking-widest text-white/20 uppercase select-none whitespace-nowrap transform rotate-6"
                      >
                        {street.name}
                      </div>
                    ))}
              ...
              {mapMode === 'satellite' && (
                <div className="absolute inset-0 pointer-events-none">
                  ...
                  <div 
                    className="absolute inset-0 pointer-events-none mix-blend-overlay opacity-40 transition-all duration-300"
                    style={{
                      backgroundImage: `
                  ...
                  <svg 
                    viewBox="0 0 100 100" 
                    className="absolute inset-0 w-full h-full pointer-events-none opacity-50 text-emerald-500/20"
                  >
                  ...
                  {cityDetailedFeatures.satelliteZones.map((zone, idx) => (
                    ...
                    >
                      <span className="text-[7px] font-mono tracking-widest text-emerald-400/20 uppercase whitespace-nowrap">
                        {zone.name}
                      </span>
                    </div>
                  ))}
                  {/* Simulated orbital HUD indicators or sensor contours */}
                  <div className="absolute bottom-16 left-4 text-[7px] font-mono text-emerald-500/40 flex flex-col items-start gap-0.5 select-none uppercase tracking-wider">
              ...
              {showGrid && (
                <div className="absolute inset-0 grid grid-cols-10 grid-rows-10 pointer-events-none opacity-40">
                  {Array.from({ length: 100 }).map((_, i) => (
                    <div key={i} className="border-[0.5px] border-white/5 font-mono text-[7px] text-white/5 p-1" />
                  ))}
                </div>
              )}
            </div>

            {/* Static Visual Scale Bar (Dynamic text calculated via current city's meters-per-unit metrics) */}
            <div className="absolute bottom-4 left-4 z-20 flex flex-col items-start gap-1 select-none pointer-events-none bg-black/75 backdrop-blur-md px-2 py-1.5 rounded-sm border border-white/10" id="map-scale-bar">
              <span className="font-mono text-[8px] text-white/65 tracking-widest uppercase font-bold">
                SCALE: {10 * cityMetersPerUnit}m
              </span>
              <div className="w-[72px] h-1 border-b border-x border-white/60 flex justify-between">
                <div className="w-[0.5px] h-0.5 bg-white/40" />
                <div className="w-[0.5px] h-0.5 bg-white/40" />
              </div>
            </div>

            {/* Simulated Landmarks Text Markers (placed outside filter to maintain readable contrast) */}
            {cityMapThemes.landmarks.filter((l: any) => !l.type).map((landmark: any, i: number) => (
              <div
                key={i}
                style={{ left: `${landmark.x}%`, top: `${landmark.y}%` }}
                className="absolute -translate-x-1/2 -translate-y-1/2 pointer-events-none text-white/30 font-serif italic text-[11px] font-light text-center select-none z-10"
              >
                ⋄ {landmark.name}
              </div>
            ))}
            ...
            {isMeasuring && measureStart && (
              <svg className="absolute inset-0 w-full h-full pointer-events-none z-20">
                ...
                <text
                  ...
                  style={{ textShadow: '0 1px 3px rgba(0,0,0,0.95), 0 1px 3px rgba(0,0,0,0.95)' }}
                >
                  [A]
                </text>
                ...
                {measureEnd && (
                  <>
                    ...
                    <circle 
                      ...
                      cy={`${measureEnd.y}%`} 
                      r="4" 
                      className="fill-red-500 stroke-white stroke-[1.5]" 
                    />
                    {/* Point B text tag */}
                    <text
                      x={`${measureEnd.x}%`}
              ...
              return (
                ...
                >
                  <button
                    ...
                    className={`group relative flex items-center justify-center rounded-full shadow-lg transition-all duration-300 pointer-events-auto transform hover:scale-125 cursor-pointer ${
                      isActive 
                        ? 'w-10 h-10 bg-red-900 border-2 border-red-500 z-30 scale-110' 
                        : isStar 
                          ? 'w-8.5 h-8.5 bg-red-600 hover:bg-red-700 border border-white/25 text-white' 
                          : isBibGourmand
                            ? 'w-8 h-8 bg-amber-600 hover:bg-amber-700 border border-white/20 text-white'
                            : 'w-7 h-7 bg-stone-800 hover:bg-stone-700 border border-white/10 text-white/80'
                    }`}
                  ...
                  >
                    ...
                    {/* Tooltip Hover Label */}
                    <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1 bg-[#0a0a0a] border border-white/10 text-white text-[10px] font-mono uppercase tracking-wider rounded shadow-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
            ...
            {highlightedRestaurant && (
              <div 
                className="absolute bottom-4 left-4 right-4 sm:left-auto sm:right-4 sm:w-80 bg-[#0a0a0a]/95 backdrop-blur-md border border-white/10 shadow-2xl rounded p-4.5 z-30 animate-fade-in text-left"
              ...
              >
                <div className="flex gap-3">
                  ...
                  <img
                    ...
                    className="w-16 h-16 object-cover rounded shrink-0 border border-white/10 grayscale hover:grayscale-0 transition-all duration-300"
                    referrerPolicy="no-referrer"
                  />
                  <div className="min-w-0 flex-1">
                    <div className="flex justify-between items-start gap-1">
                      <span className="text-[8px] tracking-wider font-mono text-[#777] uppercase font-bold">
                        {highlightedRestaurant.cuisine}
                      </span>
                      {highlightedRestaurant.stars > 0 ? (
                        <span className="text-red-500 font-serif text-xs font-bold shrink-0">
                          {Array.from({ length: highlightedRestaurant.stars }).map(() => '✻').join('')}
                        </span>
                      ) : highlightedRestaurant.distinction === 'BIB_GOURMAND' ? (
                        <span className="text-red-400 text-[10px] font-bold">☺ Bib</span>
                      ) : (
                        <span className="text-white/40 text-[9px] uppercase font-bold font-mono">Selected</span>
                      )}
                    </div>
                    <h4 className="font-serif font-light text-sm text-white mt-1 uppercase tracking-wide truncate leading-tight">
                      {highlightedRestaurant.name}
                    </h4>
                    <p className="text-[10px] text-white/60 mt-0.5 font-light truncate">
                      Chef {highlightedRestaurant.chef}
                    </p>
                    <p className="text-[10px] text-white/40 italic line-clamp-1 mt-1 font-serif font-light">
                ...
                {/* Micro Actions */}
                <div className="mt-3.5 pt-3 border-t border-white/5 flex items-center justify-between text-[9px] font-mono tracking-wider">
                  <span className="text-white/40 font-light">Y: {highlightedRestaurant.coordinates.y} X: {highlightedRestaurant.coordinates.x}</span>
                  <button
                    onClick={() => onSelectRestaurant(highlightedRestaurant)}
                    className="flex items-center gap-1 font-bold text-red-500 hover:text-red-400 transition-colors select-none cursor-pointer"
                  >
        ...
        <div className="w-full lg:w-80 flex flex-col gap-4">
          <div className="bg-[#0a0a0a] border border-white/5 rounded-xl p-5 shadow-2xl flex flex-col">
            <h3 className="font-serif font-light text-base text-white flex items-center gap-2 uppercase tracking-wider">
            ...
            <div className="space-y-2 mt-4 max-h-[380px] overflow-y-auto pr-1">
                  ...
                  <button
                    ...
                    className={`w-full text-left p-2.5 rounded border transition-all flex items-center gap-3 cursor-pointer ${
                      isSelected
                        ...
                        : 'bg-[#050505] hover:bg-white/5 border-white/5 hover:border-white/10 text-white/80'
                    }`}
                  >
                    ...
                    <div className={`w-8.5 h-8.5 rounded-full flex items-center justify-center shrink-0 font-bold transition-colors ${
                      ...
                      {restaurant.stars > 0 ? (
                        <span className="text-amber-300 text-xs font-bold leading-none mb-0.5">✻</span>
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {restaurant.stars > 0 && (
                          <span className="text-red-500 font-serif text-[9px] font-bold">
                            {restaurant.stars}★
                          </span>
                        )}
                      </div>
                      <h4 className="font-serif font-light text-xs truncate text-white uppercase tracking-wide mt-0.5">
                        {restaurant.name}
                      </h4>
                      <p className="text-[9px] text-white/40 truncate font-mono font-light">
              ...
              {cityRestaurants.length === 0 && (
                <div className="text-center py-8 text-xs font-serif italic text-white/40 font-light">
